...
local buttonsize = SettingsCF["actionbar"].button_size
local flyoutsize = SettingsCF["actionbar"].button_size
local buttonspacing = SettingsDB.Scale(3)
local borderspacing = SettingsDB.Scale(2)
...
local function SkinFlyoutButton(button)
	...
	SettingsDB.CreateTemplate(button.skin)
...
local function SkinButton(button, colorr, colorg, colorb)
	...
	if button.actionButton then
		SettingsDB.CreateTemplate(button.actionButton)
...
local function SkinFlyoutTray(tray)
	...
	SettingsDB.CreateTemplate(closebutton)
	...
	tray:SetPoint("BOTTOM", parent, "TOP", 0, buttonspacing + SettingsDB.Scale(1))
	closebutton:SetPoint("BOTTOM", tray, "TOP", 0, SettingsDB.Scale(1))
