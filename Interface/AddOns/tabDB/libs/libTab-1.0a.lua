	...
	libTab = {
		...
		tabHook = function (self, tabsId)
			for key in libTab:tabFilteredPairs(libTab.Data[tabsId].tab, "number") do
				libTab.Data[tabsId].tab[key]["OnShow"] = getglobal(libTab.Data[tabsId].tab[key].Frame):GetScript("OnShow") or nil
				getglobal(libTab.Data[tabsId].tab[key].Frame):SetScript("OnShow", function(self, ...) libTab.tabOnShow(self, tabsId, ...) end)
				libTab.Data[tabsId].tab[key]["OnHide"] = getglobal(libTab.Data[tabsId].tab[key].Frame):GetScript("OnHide") or nil
				getglobal(libTab.Data[tabsId].tab[key].Frame):SetScript("OnHide", function(self, ...) libTab.tabOnHide(self, tabsId, ...) end)
			end
		end,
		...
		initialize = function (self, tabsId, tabsTable)
			...
			for key, value in libTab:tabOrderedPairs(tabsTable, "order") do
				...
				libTab.Data[tabsId].tab[i].offsetY = value.offsetY or 0
					...
					UIPanelWindows[value.Frame]["width"] = getglobal(value.Frame):GetWidth()+(libTab.TabOffsetX+value.offsetX)
